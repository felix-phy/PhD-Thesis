...
\begin{align}
    ...
    &= -m_{\mathrm{f}}\overline{\psi}\psi(1 + \frac{1}{v}\frac{h}{\sqrt{2}}) \label{YukawaLagrangian}
\end{align}
\\
...
By determining the frequency of occurence of physics processes in which the coupling between the Higgs boson and charm quark appears, $y_{\mathrm{c}}$ may be measured indrectly. As such, a suitable process must be found that can be measured by an experiment such as CMS. These fall into two categories. The first consists of processes in which a Higgs boson decays into a charm and anti-charm quark pair (\ccbar) while the second consists of processes in which a Higgs boson is produced in association with a charm quark. The latter category of processes is the focus of this work. 
...
\begin{align}
    \Gamma_{\mathrm{H}} =& \Gamma_{\mathrm{H}}^{\mathrm{SM}} \sum_{f} \kappa_f^2 \mathrm{BR}_f^{\mathrm{SM}} \label{TotalModifiedWidthSum} \\ i should fully write this out
...
\begin{align}
    \mu_{if} = \frac{\kappa_{\mathrm{c}}^2}{\kappa_{\mathrm{H}}^2}
\end{align}
Using the flat direction approach discussed in \cite{PhysRevD.100.073013} and \cite{PhysRevD.103.095023}, a simplification of $\kappa_{\mathrm{H}}$ can be introduced. This approach is based on the finding that, when performing fits to existing Higgs production and decay rates, increases in the Yukawa couplings of light quarks including the charm quark could be compensated by increases in the couplings of the gauge bosons and heavy fermions. This is referred to as a "flat direction" in the fit, where observed Higgs production and decay rates can be modeled equally well for any value of $\kappa_{\mathrm{c}}$. Thus the authors replace the individual modifiers in the sum of \autoref{TotalModifiedWidthSum} with a single modifier $\kappa$ and write 
