...
\begin{itemize}
    ...
    \end{equation}
    where $F_0$ is the nominal value of the observable in question and $F_i$ is the varied value associated with one of the $N_{\text{par}}$ parameter variations \cite{Butterworth_2016}. A yield uncertainty of approximately 6.5\% on the total yield of the background processes is associated with this uncertainty. 
    \item An uncertainty related to the choice of the strong coupling constant $\alpha_s$ in the Monte Carlo simulation. This uncertainty is associated with a yield change of approximately 2\% on the total background process contributions. 
    ...
    \item A shape uncertainty related to the application of a pile-up reweighting procedure, with which the pile-up profile of simulation is matched to that of data. This uncertainty is described with a single nuisance parameter. The uncertainty on process yields that is introduced by this uncertainty is almost negligible.
\end{itemize}
...
\begin{figure}
    \begin{subfigure}{1\textwidth}
        \centering
        \includegraphics[width=0.6\textwidth]{figures/UnrolledDisc_mH_100To130.pdf}
    ...
    \begin{subfigure}{1\textwidth}
        \centering
        \includegraphics[width=0.6\textwidth]{figures/UnrolledDisc.pdf}
