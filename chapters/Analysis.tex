...
\begin{itemize}
    ...
    \item An uncertainty related to the choice of the strong coupling constant $\alpha_s$ in the Monte Carlo simulation. 
    \item Shape uncertainties related to the tuning of the parton showering used in the Monte Carlo generator. These are implemented by varying parameters related to initial-state radiation and final-state radiation independently, introducing two nuisance parameters. 
    \item A shape uncertainty related to the application of a pile-up reweighting procedure, with which the pile-up profile of simulation is matched to that of data. This uncertainty is described with a single nuisance parameter. 
\end{itemize}
...
\begin{itemize}
    ...
    \item A shape uncertainty related to the smearing method used to correct the jet energy resolution. This is described via a single nuisance parameter.
    \item Shape uncertainties related to the calibration of the jet flavour-tagging algorithm that is used. The effect of these are captured via 13 nuisance parameters and include for example uncertainty sources related to the individual phase spaces targeted in the calibration or the limited set of avaialble data. 
    \item A shape uncertainty related to the pile-up identification method that is used. This uncertainty is described with a single nuisance parameter.
    \item Shape uncertainties related to the muons used in the analysis. The response of the muon identification criteria, the isolation as well as the muon trigger path in simulation is calibrated to match data. Uncertainties associated with this calibration are combined into a single nuisance parameter. 
