...
\\
A number of systematic uncertainty sources can influence the reducible background estimation. This includes the effects of statistical uncertainties on each bin of the muon misidentification rate, effects related to differences in the background composition between different control regions as well as the discussed simplication introduced in the estimation strategy. However, due to the subdominant nature of this background and the fact that its contributions are minimal in the phase space of the signal, these effects are not propagated to the statistical evaluation. Instead a conservative rate uncertainty of 50\% is introduced to approximate their effect.
...
The sources of uncertainty in the model of the signal and background processes are described in this section. These consist of two types. The first are shape uncertainties that may change the normalisation as well as the shape of the distributions in question. These uncertainties are captured by creating variations of these distributions that are interpreted as $\pm 1\sigma$ variations. In the statistical evaluation process, an interpolation is performed between the nominal distributions as well the variations to characterise their effect. as the The second type consists of normalisation uncertainties. These only affect the normalisation of distributions without affecting the shape and can thus be captured as a single, real paarameter. Both types of uncertainties are associated with a respective nuisance parameter in the statistical model introduced in \autoref{sec:statisticalModel}.
...
\begin{itemize}
    \item Shape uncertainties related to the choice of the normalisation and factorisation scales $\mu_R$ and $\mu_F$. These are varied independently by factors of 2 and 0.5 using a reweighting technique that is applied to the simulation, thus introducing two shape-changing nuisance parameters. 
