...
\end{enumerate}
The reconstructed Higgs boson candidate mass distribution in simulated \cHZZ events can be seen in \autoref{fig:cHHiggsMass}. As expected, a peak around the known Higgs mass $m_{H}$ = 125 \cite[PhysRevD.110.030001] GeV can be oberserved, with an elongated tail towards lower masses that originate from increasingly off-shell Z candidate contributions. 
...
\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{figures/jetSelectionEfficiency.pdf}
...
\begin{table}[H]
    ...
    \label{table:SignalSamples}
        ...
        \midrule 
        \midrule
        \cHZZ 4FS FxFx & 0.00916 fb & 4,854,537 \\
        \cHZZ 3FS & 0.008065 fb & xx \\
        \cHZZ 4FS & 0.004987 fb & xx \\
        \bottomrule
        \end{tabular} 
    \end{adjustbox}
\end{table}
...
\begin{table}[H]
    ...
    \label{table:IrreducibleBackgroundSamples}
        ...
        \midrule 
        bH 5FS FxFx & 0.7095 fb & 3,869,701 \\
        bH 4FS &  0.365 fb & xx \\
        bH 5FS & 0.6573 & xx \\
        \midrule
        WZ$\rightarrow3\ell\nu$ & 4.43 pb & xx \\
...
\end{align}
\\
By applying this strategy, an inclusive yield of 6.59 events for the reducible background in the 4P region is observed. In the signal region around the Higgs mass peak of 120 GeV \textless m(H) \textgreater 130 GeV, this reduces to a yield of 0.42 events. However, the derived distributions of observables are significiantly discontinuous. This arises from the strong influence of statistical fluctuations on the small yields of the individual control regions. The effects of these fluctuations are compounded by the subtractions performed in \autoref{eq:reducibleMasterEquation}, which introduce a significant number of bins with negative, unphysical yields. Given the subdominant nature of this background, a simplifcation to the estimation is made to resolve this problem. Specifically, the shapes of the distributions of observables in the 2P2F region, the most populated control region in the reducible background estimation, are taken and scaled to the yield of the full estimation. This approach is validated by a comparison of the 2P2F and 3P1F flavour tagging disciminator shapes, which show reasonable similarity as seen in \autoref{}. The contribution of the reducible background estimation to distributions of observables that is derived in this way can be seen in the following section. \\
\\
A number of systematic uncertainty sources can influence the reducible background estimation. This includes the effects of statistical uncertainties on each bin of the muon misidentification rate, effects related to differences in the background composition between different control regions as well as the discussed simplication introduced in the estimation strategy. However, due to this backgrounds minimal contribution in the phase space of the signal, these effects are not propagated to the statistical evaluation. Instead a conservative rate uncertainty of 100\% is introduced, allowing the yield of this background to effectively be determined by a best fit value in the statistical evaluation described in \autoref{sec:StatisticalEvaluation}
...
\subsection{Additional uncertainties}
\begin{itemize}
...
\section{Results}
\label{sec:SMResult}
In this section, the result of the presented methods is discussed. The statistical evaluation is performed on the unrolled discriminator distribution described in \autoref{sec:StatisticalEvaluation}. This distribution can be seen in \autoref{fig:UnrolledDisc}. With the presented method, an expected 95\% CL \CLs upper limit of  $\sigma$BR(\cHZZ)$ = 4000$ is set for the 2018 dataset of the CMS detector. By applying the prescription derived in \autoref{sec:sec:kappaframework}, this corresponds to a limit on $\kappa_c$ of  $\kappa_c = $.
