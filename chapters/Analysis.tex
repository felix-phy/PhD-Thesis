...
\end{enumerate}
The mis-identification rate of the probe muon, in bins of the probe muon \pt, that is determined in this way can be seen in \autoref{fig:misIDRate}. However, the contribution from processes that indeed produce three muons in the hard-scattering  must be subtracted. This consists primarily of WZ$\rightarrow3\ell\nu$ processes that artifically inflate the calculated mis-identification rate at higher probe muon \pt. This contribution is subtracted using simulation. It is this corrected version of the mis-identification rate that is used in the following section. 

\subsection{Application of muon mis-identification rate}
...
\begin{enumerate}
    ...
    \begin{align}
        N_{\mathrm{4P}}^{\mathrm{(2P2F)}} = \sum_{k}^{\mathrm{2P2F}} \frac{f_k^{(3)}}{(1-f_k^{(3)})} \frac{f_k^{(4)}}{(1-f_k^{(4)})},
    \end{align}
    ...
    \begin{align}
        N_{\mathrm{4P}}^{\mathrm{(3P1F)}} = \sum_{k}^{\mathrm{3P1F}}\frac{f_k^{(4)}}{(1-f_k^{(4)})},
    \end{align}
...
\end{enumerate}
To obtain the total contribution of the AR to the 4P region, potential overlap between the 2P2F and 3P1F regions must be accounted for.
