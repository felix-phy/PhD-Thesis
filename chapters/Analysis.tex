...
For the statistical inference process, both the \textsf{DeepJet} CvsB and CvsL discriminators are used in a pseudo two-dimensional fit. Specifically, an \textit{unrolling} process is applied to events to project the two-dimensional information given by the CvsB and CvsL discriminators into a single discriminator. This involves constructing a histogram of the CvsB discriminator in bins of the corresponding CvsL value. The resulting histogram can be seen in \autoref{fig:UnrolledFinalHist}. It is on this distribution that the statistical inference process described in the following is performed. This consists of constructing a statistical model with which a fit may be performed as described in \autoref{sec:statisticalModel} as well as an uncertainty model, as described in \autoref{sec:uncertaintyModel}\\
\\
...
\section{Results}
\label{sec:SMResult}
In this section, the results obtained by performing the statistical evaluation described in \autoref{sec:StatisticalEvaluation} are discussed. A number of different scenarios are considered and expected 95\% CL \CLs upper limit on $\sigma$BR(\cHZZ) and $\kappa_c$ are presented for each scenario. Additionally, the impacts of the individual uncertainties source on the fitting procedure performed during the statistical evaluation are estimated. Finally, a number of potential improvements to the analysis are discussed. 

The nominal analysis scenario is that in which the unrolled CvsL and CvsB discriminator distribution of the selected jet for \cHZZ candidate events is used as a discriminator for the 2018 dataset of the CMS detector. This distribution, for which no criteria on the mass of the Higgs boson candidate are applied, can be seen in \autoref{fig:UnrolledDisc}. With the presented method, an expected 95\% CL \CLs upper limit of $\sigma$BR(\cHZZ)$ = 3069$ is set. By applying the prescription derived in \autoref{sec:kappaframework}, this corresponds to an expected 95\% CL \CLs upper limit of $\kappa_c = xx$. To better understand the impact of the nuisance parameters on this result, a series of fits to an Asimov dataset is performed. This Asimov dataset consists of the nominal background process contributions as well as a \cHZZ signal contribution, which is injected with a signal strength $r$ corresponding to the derived upper limit. In such a signal-strength fit, the profile likelihood ratio is maximised to find a best fit value of $\hat{r}$ for the signal strength in the given dataset. Following this is a series of fits where the individual nuisance parameters $\theta_i$ are varied by $\pm \sigma$ to find the best fit values $\hat{r}_{\pm\theta_i}$. The size of the differences $\Delta \hat{r}_i = \hat{r} - \hat{r}_{\pm\theta_i}$ thus gives an indication of the uncertainty on $\hat{r}$ introduced by each nuisance parameter. With this technique, the leading uncertainties that limit the sensitivity the of the 95\% CL \CLs upper limit fit can thus be identified. These can be seen in \autoref{fig:ImpactsSM}, which shows the $\Delta \hat{r}_i$ values of the leading 30 (of a total of 76) uncertainty sources. As can be seen, the flavour scheme uncertainty from the \cHZZ simulation is clearly the leading uncertainty source. Given the relatively large yield changing effect associated with this uncertainty and the very small expected signal strength of the \cHZZ process, this an expected behaviour. Generally it is unsurprising then, that other leading uncertainties sources produce yield changing effects in the signal and background contributions. These include for example uncertaintiess associated with the PDF used in simulation, the normalisation of the reducible backgrounds, the luminosity of the LHC and the k-factor applied to the qq$\rightarrow$ZZ process. This can again be explained by the fact that the fit sensitivity is primarily limited by the very small expected signal strength. Given that the discriminating observable is derived from the properties of the selected jet of the \cHZZ candidates, the significant impact of the uncertainties related to jet calibration and flavour tagging methods is also expected. The distributions associated with the 30 leading uncertainty sources can be seen in \autoref{appendix:uncertaintyDistributionsSM}. Only the shape changing uncertainties are included since the normalisation uncertainties simply corresponds to a scaling of the distributions . \\
\\
Another analysis scenario is one which ... $\sigma$BR(\cHZZ)$ = 983$ (signal 100-130)

Inclusive (exclusive) extrapolation to Run-2 ... $\sigma$BR(\cHZZ)$ = 2078$ ($\sigma$BR(\cHZZ)$ = 645$)

Inclusive (exclusive) extrapolation to HL-LHC  ... $\sigma$BR(\cHZZ)$ = 613$ ($\sigma$BR(\cHZZ)$ = 156$)


To make a comparison to the published cH(WW) and cH($\gamma\gamma)$ results, a statistical extrapolation to the full Run-2 dataset of the CMS detector is performed. This consists of scaling the prediction of the 2018 dataset to the 137 $fb^{-1}$ of luminosity of the full Run-2 dataset. However, it should be noted that this extrapolation neglects any changes in detector operating conditions and assumes no changes in any of the uncertainty sources. In this case, an expected 95\% CL \CLs upper limit of $\sigma$BR(\cHZZ)$ = 2864$ is derived, which corresponds to an expected 95\% CL \CLs upper limit of $\kappa_c = 497$. ...\\
\\
Similarly, the result may be extrapolated to the expected 3000 fb${-1}$ dataset of the High-Luminosity LHC (HL-LHC) \cite{}. Again, it should be noted that this neglects changes in the CMS detector and operating conditions, which are expected to differ significantly during HL-LHC operation \cite{}. A result of an expected 95\% CL \CLs upper limit of $\sigma$BR(\cHZZ)$ = 2864$ is derived in this case. This corresponds to an expected 95\% CL \CLs upper limit of $\kappa_c = 497$. ...

\begin{figure}
    \centering
    \includegraphics[width=0.9\textwidth]{figures/UnrolledDisc.pdf}
    \caption{The unrolled discriminator distribution representing the CvsB discriminator in three bins of the CvsL discriminator for the selected jet of the reconstructed \cHZZ candidates. As can be seen in the lower panel, the relative contribution of a \cHZZ signal in each bin generally increases with increasing bin number. The region most enriched with signal are the last bins on the right side of the distributions, which as expected, corresponds to selected jets with both a high CvsL and CvsB value. The \cHZZ signal contribution is scaled by a factor of $10^{3}$ to improve its visibility.}
