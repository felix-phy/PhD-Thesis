...
\\
To capture uncertainties associated with the choice of a particular flavour scheme, additional \cHZZ samples are used. These specifically simulate the \cHZZ process in the 3FS and 4FS, without the use of FxFx merging, effectively capturing the cases where a charm \textit{must} originate from gluon splitting or the proton respectively. From these samples, an uncertainty envelope is constructed for the discriminators distribution in the statistical evaluation presented in \autoref{sec:StatisticalEvaluation}. This envelope is constructed by taking the discrepancy between the 3FS and 4FS samples and applying it as an uncertainty band the the nominal 4FS FxFx sample. An overview of all used signal samples can be seen in \autoref{table:SignalSamples}. \\
\\
\begin{table}[H]
    \centering
